...
/**
 * Class \Sunnysideup\Recipes\Pages\RecipeController
 *
 * @property Recipe $dataRecord
 * @method Recipe data()
 * @mixin Recipe
 */
class RecipeController extends BlogPostController
{
    ...
    public function RelatedPosts()
    {
        ...
        foreach ($this->Categories() as $category) {
            ...
            foreach ($posts as $post) {
                if (0 === count($mustHaveIDs) || in_array($post->ID, $mustHaveIDs, true)) {
                    $al->push($post);
                }
            }
        }
    ...
    public function TagsFiltered()
    {
        return $this->Tags()->filter(['ShowOnSite' => true]);
    }

    public function CategoriesFiltered()
    {
        return $this->Categories()->filter(['ShowOnSite' => true]);
    }
}
